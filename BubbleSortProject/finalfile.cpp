...
void drawArray(const vector<int>& arr, int sortedTill = -1, int idx1 = -1, int idx2 = -1) {
    ...
    int width = max(1, 150 / (int)arr.size()); // fit screen
...
// Selection sort with one movement per iteration
void selectionSortVisual(vector<int>& arr) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        this_thread::sleep_for(chrono::milliseconds(10)); // adjust speed
    }
}
...
int main() {
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> lenDist(50, 100); // array length
    int n = lenDist(gen);
